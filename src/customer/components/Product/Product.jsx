...
export default function Product() {
  ...
  const handleFilter = (value, sectionId) => {
    ...
    if (filterValues.includes(value)) {
      // Nếu giá trị đã có, loại bỏ nó
      filterValues = filterValues.filter(item => item !== value);
    } else {
      // Nếu giá trị chưa có, thêm vào
      filterValues.push(value);
    }

    if (filterValues.length > 0) {
      // Cập nhật lại tham số tìm kiếm với danh sách mới
      searchParams.set(sectionId, filterValues.join(","));
    } else {
      // Nếu danh sách trống, xóa tham số tìm kiếm
                            ...
                            <div className="space-y-4">
                              <FormControl>
                                <RadioGroup
                                  ...
                                  onChange={(e) => handleRadioFilterChange(e, section.id)} // Sử dụng `section.id` chứ không phải `section.Id`
                                >
                                  {section.options.map((option, optionIdx) => (
                                    <FormControlLabel
                                      value={option.value}
